...
class TransformerMultipath(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
                 ...
                 return_intermediate_dec=False, skip_encoder=False, n_path=3, num_queries = 100):
        super().__init__()
        ...
        self.decoder = TransformerDecoderMultipath(decoder_layer, num_decoder_layers, decoder_norm,
                                          return_intermediate=return_intermediate_dec, n_path = n_path, num_queries = num_queries)
...
class TransformerMultipath430(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        decoder_layer = TransformerDecoderLayerMultipath430(d_model, nhead, dim_feedforward,
                                                dropout, activation, normalize_before, flag_decouple_pos_content,flag_no_tripath = flag_no_tripath)
        decoder_norm = nn.LayerNorm(d_model)
        self.decoder = TransformerDecoderMultipath430(decoder_layer, num_decoder_layers, decoder_norm,
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
                 num_decoder_layers=6, dim_feedforward=2048, dropout=0.1,
                 activation="lrelu", normalize_before=False,
                 return_intermediate_dec=False, skip_encoder=False):
        super().__init__()
        #return intermediate dec set to True
        if(not skip_encoder):
          encoder_layer = TransformerEncoderLayer(d_model, nhead, dim_feedforward,
    ...
    def forward(self, src, mask, query_embed, pos_embed, src_attention_mask=None):
        #input shape: HWxNxC
        hw, bs, c = src.shape
        #src = src.flatten(2).permute(2, 0, 1)
        #pos_embed = pos_embed.flatten(2).permute(2, 0, 1)
...
class TransformerDecoderMultipath(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False, n_path = 3, num_queries = 100):
        super().__init__()
        ...
        for j in range(self.num_layers):
            ...
            for iter1 in range(self.n_path):
                ...
                for id in idlist:
                    #output selfatt: 100,1,192
                    # feature_list.append((output_selfatt[id] + primitive_type_embed[id + 1]))
                    feature_list.append((output_list[id] + primitive_type_embed[id + 1]))
                feature_list.append(memory)
                memory_cross.append(torch.cat(feature_list, dim=0))
            ...
            for i in range(self.n_path):
                ...
                output_stage2_res.append(self.layers_list[i][j](output_list[i], memory_cross[i], tgt_mask=tgt_mask,
                            memory_mask=memory_mask,
                            tgt_key_padding_mask=tgt_key_padding_mask,
                            memory_key_padding_mask=memory_key_padding_mask_cross,
                            pos=pos_cross_list[i], query_pos=query_pos_list[i], stage = 2))
            ...
            for i in range(self.n_path):
                ...
                #             memory_key_padding_mask=memory_key_padding_mask_cross,
                #             pos=pos_cross, query_pos=query_pos_list[i], stage = 2, tgt2 = output_selfatt[i])
                output_stage3_res.append(self.layers_list[i][j](output_list[i], memory_cross[i], tgt_mask=tgt_mask,
                            memory_mask=memory_mask,
                            tgt_key_padding_mask=tgt_key_padding_mask,
                            memory_key_padding_mask=memory_key_padding_mask_cross,
                            pos=pos_cross_list[i], query_pos=query_pos_list[i], stage = 3))
...
class TransformerDecoderMultipath430(nn.Module):
...
class TransformerDecoderLayerMultipath(nn.Module):
    ...
    def forward_pre_stage2(self, tgt2, memory,
                    tgt_mask: Optional[Tensor] = None,
                    memory_mask: Optional[Tensor] = None,
                    tgt_key_padding_mask: Optional[Tensor] = None,
                    memory_key_padding_mask: Optional[Tensor] = None,
    ...
    def forward(self, tgt, memory,
        ...
        if stage == 1:
            ...
            return self.forward_pre_stage2(tgt, memory, tgt_mask, memory_mask,
                                    tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos)
        elif stage == 3:
            tgt = self.norm3(tgt)
...
class TransformerDecoderLayerMultipath430(nn.Module):
...
def build_transformer_debug(args):
...
def build_transformer_tripath(args):
    if args.ori_tripath:
        ...
        return TransformerMultipath430(
            d_model=args.m * 6, #32x6
            dropout=args.dropout,
            nhead=args.nheads,
            dim_feedforward=args.dim_feedforward,
