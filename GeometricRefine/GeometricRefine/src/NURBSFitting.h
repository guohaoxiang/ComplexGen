...
#include "HOptimizer/HOptimizer.h"
#include "nanoflann.hpp"
#include <string>
#include <iostream>
#include <fstream>

typedef gte::Vector<3, double> vec3;

class MyGeometry
{
public:
	void set_points(const std::vector<vec3> &points, const std::vector<double> &weights = std::vector<double>())
	{
		if (points.empty())
			return;
	...
	void set_points(const std::vector<std::vector<vec3>> &points, const std::vector<std::vector<double>> &weights = std::vector<std::vector<double>>())
	{
	...
	void set_addon(const std::vector<vec3> &points, const std::vector<double> &weights = std::vector<double>())
	{
		if (points.empty())
			return;
	...
	virtual void update_solution(const std::vector<double> &variables) = 0;
	virtual void funcgrad(const std::vector<double> &variables, double &func_value, std::vector<double> &gradient) = 0;
	virtual void write_obj(const std::string &filename, int u_samplerate, int v_samplerate) = 0;

	double &smoothness() { return smoothness_weight; }
	const double &smoothness() const { return smoothness_weight; }

// protected:
	std::vector<vec3> input_points;
	std::vector<double> input_point_weights;

	std::vector<std::vector<vec3>> input_grid_points;
	std::vector<std::vector<double>> input_grid_point_weights;

	std::vector<vec3> additional_points;
	std::vector<double> additional_point_weights;
...
public:
	...
	void optimize(MyGeometry &mygeometry, const std::vector<vec3> &points, int max_iter)
	{
	...
	void optimize(MyGeometry &mygeometry, const std::vector<vec3> &points, const std::vector<double> &point_weights, int max_iter)
	{
	...
	void optimize(MyGeometry &mygeometry, const std::vector<vec3> &points, const std::vector<double> &point_weights,
	...
	void optimize(MyGeometry &mygeometry, const std::vector<std::vector<vec3>> &points, int max_iter)
	{
		mygeometry.set_points(points);
		std::vector<double> variables;
		mygeometry.initialize(variables);
	...
	void optimize(MyGeometry &mygeometry, const std::vector<std::vector<vec3>> &points, const std::vector<std::vector<double>> &weights, int max_iter)
	{
	...
	void optimize(MyGeometry &mygeometry, const std::vector<std::vector<vec3>> &points, const std::vector<std::vector<double>> &weights,
				  const std::vector<vec3> &addon_points, const std::vector<double> &addon_weights, int max_iter)
	{
		mygeometry.set_points(points, weights);
		mygeometry.set_addon(addon_points, addon_weights);
	...
	static void eval_funcgrad(const size_t n_vars, const std::vector<double> &variables, double &func_value, std::vector<double> &gradient, void *user_pointer)
	{
		static_cast<MyGeometry *>(user_pointer)->funcgrad(variables, func_value, gradient);
	}
	static void newiteration(const size_t niter, const size_t call_iter,
...
class MyNURBSCurve : public MyGeometry
{
public:
	MyNURBSCurve(int degree, bool closed, const std::vector<vec3> &ctl_points)
	...
	void set_opt_weight(bool opt)
	{
		opt_weight = opt;
	}

	void set_opt_addon(bool opt, int _sample_rate = 100)
	{
	...
	void update_solution(const std::vector<double> &variables)
	{
		int num_free_controls = is_closed ? mycurve->GetNumControls() - mycurve->GetBasisFunction().GetDegree() : mycurve->GetNumControls();
		vec3 v;
	...
	void funcgrad(const std::vector<double> &variables, double &func_value, std::vector<double> &gradient)
	{
		if (!mycurve)
			return;
		...
		if (opt_addon && additional_points.empty() == false)
		{
			...
			{
				double du = u_min + (u_max - u_min) * i / (is_closed ? sample_rate : sample_rate - 1);
	...
	void write_obj(const std::string &filename, int u_samplerate, int v_samplerate)
	{
	...
	void write_nurbs(const std::string &filename)
	{
		std::ofstream objfile(filename);
	...
	///////////////////////////////////////////////////////
	inline size_t kdtree_get_point_count() const { return sample_points.size(); }
	inline double kdtree_get_pt(const size_t idx, const size_t dim) const
	{
		return sample_points[idx][(int)dim];
	}
	template <class BBOX>
...
// protected:
	...
	bool is_closed, opt_weight, opt_addon;
	int sample_rate;
...
{
public:
	CurveFitting(const std::vector<vec3> &points, const std::vector<double> &weights, const std::vector<vec3> &addon_points,
				 const std::vector<double> &addon_weights, bool closed, int degree, int num_ctrl, int num_iter = 100, int sample_rate = 100,
				 double smoothness_weight = 0)
	{
		...
		curvelen.reserve(points.size() + 1);
		double len = 0;
		...
		if (closed)
		{
			len += gte::Length(points.front() - points.back());
			curvelen.push_back(len);
		}
		...
		{
			double t = i * len / (closed ? num_ctrl : num_ctrl - 1);
			while (t >= curvelen[k] - 1.0e-6 && k + 1 < curvelen.size())
				k++;
			double shortlen = curvelen[k] - curvelen[k - 1] + 1.0e-12;
			//controls[i] = (t - curvelen[k - 1]) / shortlen * points[k - 1] + (curvelen[k] - t) / shortlen * points[k % points.size()];
			controls[i] = (t - curvelen[k - 1]) / shortlen * points[k % points.size()] + (curvelen[k] - t) / shortlen * points[k - 1];
	...
	MyNURBSCurve *get_curve() { return mycurve; }
	const MyNURBSCurve *get_curve() const { return mycurve; }

// protected:
	MyNURBSCurve *mycurve;
};

class MyNURBSSurf : public MyGeometry
{
public:
	MyNURBSSurf(int u_degree, bool u_closed, int v_degree, bool v_closed, const std::vector<std::vector<vec3>> &ctl_points)
	...
	void funcgrad(const std::vector<double> &variables, double &func_value, std::vector<double> &gradient)
	{
		...
		if (opt_addon && additional_points.empty() == false)
		{
			...
			for (int i = 0; i < sample_rate; i++)
			{
				double du = u_min + (u_max - u_min) * i / (is_u_closed ? sample_rate : sample_rate - 1);
				u_basis.Evaluate(du, 0, iumin, iumax);
				for (int j = 0; j < sample_rate; j++)
				{
					double dv = v_min + (v_max - v_min) * j / (is_v_closed ? sample_rate : sample_rate - 1);
					double w = 0;
					v_basis.Evaluate(dv, 0, ivmin, ivmax);
					X.MakeZero();
					for (int iv = ivmin; iv <= ivmax; ++iv)
...
// protected:
	...
	bool is_u_closed, is_v_closed, opt_weight, opt_addon;
	int sample_rate;
	std::vector<vec3> sample_points;
};
...
{
public:
	SurfFitting(const std::vector<std::vector<vec3>> &points, const std::vector<std::vector<double>> &weights,
				const std::vector<vec3> &addon_points = std::vector<vec3>(), const std::vector<double> &addon_weights = std::vector<double>(),
				bool u_closed = false, bool v_closed = false, int u_degree = 3, int v_degree = 3, int u_num_ctrl = 5, int v_num_ctrl = 5,
				int num_iter = 100, int sample_rate = 100, double smoothness_weight = 0)
	{
		if (points.empty() ||
		...
		{
			u_curvelen[j].reserve(points.size() + 1);
			double len = 0;
			for (size_t i = 0; i < points.size() - 1; i++)
			{
			...
			if (u_closed)
			{
				len += gte::Length(points.front()[j] - points.back()[j]);
				u_curvelen[j].push_back(len);
			}
		}
		...
		for (int j = 0; j < points.front().size(); j++)
		{
			...
			{
				double t = i * u_curvelen[j].back() / (u_closed ? u_num_ctrl : u_num_ctrl - 1);
				while (t >= u_curvelen[j][k] - 1.0e-6 && k + 1 < u_curvelen[j].size())
					k++;
				double shortlen = u_curvelen[j][k] - u_curvelen[j][k - 1] + 1.0e-12;
//				u_controls[j][i] = (t - u_curvelen[j][k - 1]) / shortlen * points[k - 1][j] + (u_curvelen[j][k] - t) / shortlen * points[k % points.size()][j];
				u_controls[j][i] = (t - u_curvelen[j][k - 1]) / shortlen * points[k % points.size()][j] + (u_curvelen[j][k] - t) / shortlen * points[k - 1][j];
		...
		{
			v_curvelen.reserve(points.front().size() + 1);
			double len = 0;
			...
			if (v_closed)
			{
				len += gte::Length(u_controls.front()[j] - u_controls.back()[j]);
				v_curvelen[j].push_back(len);
			}
		}
		...
		for (int i = 0; i < u_num_ctrl; i++)
		{
			...
			{
				double t = j * v_curvelen[i].back() / (v_closed ? v_num_ctrl : v_num_ctrl - 1);
				while (t >= v_curvelen[i][k] - 1.0e-6 && k + 1 < v_curvelen[i].size())
					k++;
				double shortlen = v_curvelen[i][k] - v_curvelen[i][k - 1] + 1.0e-12;
				//controls[i][j] = (t - v_curvelen[i][k - 1]) / shortlen * u_controls[k - 1][i] + (v_curvelen[i][k] - t) / shortlen * u_controls[k % u_controls.size()][i];
				controls[i][j] = (t - v_curvelen[i][k - 1]) / shortlen * u_controls[k % u_controls.size()][i] + (v_curvelen[i][k] - t) / shortlen * u_controls[k - 1][i];
			}
		}
	...
	MyNURBSSurf *get_surf() { return mysurf; }
	const MyNURBSSurf *get_surf() const { return mysurf; }
